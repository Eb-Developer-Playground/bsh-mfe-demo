...
describe('buildRemoteRoutes', () => {
  ...
  it('attaches a guard to routes that require auth', () => {
    const routes = buildRemoteRoutes([
      loaded({
        source: '@x/checkout',
        basePath: 'checkout',
        intents: [{ id: 'cart', path: '/cart', element: 'mfe-cart', requiresAuth: true }],
      }),
    ]);

    expect(routes[0].canActivate).toHaveLength(1);
    expect(routes[0].data).toMatchObject({ requiresAuth: true });
  });
