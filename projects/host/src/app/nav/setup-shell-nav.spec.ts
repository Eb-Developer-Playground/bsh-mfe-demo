...
describe('setupShellNavigation', () => {
  ...
  it('registers routes built from all loaded contributions', async () => {
    ...
    // static login route + 2 explore intents + 1 decide intent + 1 wildcard
    expect(router.routes).toHaveLength(5);
    expect(router.routes[0]).toMatchObject({ path: 'login' });
  ...
  it('continues when a remote contribution fails to load', async () => {
    ...
    // static login route + only explore's 2 intents + wildcard
